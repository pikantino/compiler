...
async function build(options) {
    ...
    const dependenciesMaps = await Promise.all(files.map(file => transpile_1.transpile(file, options).then((deps) => createDependenciesMap(file, deps))));
...
exports.watch = watch;
function copyIndexFile(srcDir, outDir, cwd) {
...
async function updateCompilerOptions(options) {
    const compilerOptions = await readCompilerOptions(options.cwd, options.compilerOptions)
        .catch((error) => error_handler_1.handleError(error, 'Cannot read tslint.json', {}));
    return {
        ...options,
        compilerOptions
    };
}
function readCompilerOptions(cwd, customOptions) {
    return fs.readFile(path.join(cwd, './tsconfig.json'))
        .then((file) => {
        return Object.assign({}, JSON.parse(file.toString()).compilerOptions, customOptions, { module: ts.ModuleKind.ES2015 });
    });
}
